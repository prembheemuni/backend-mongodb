...
export const applyDisCount = TryCatch(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!couponDetails) return next(new ErrorHandler("Invalid Coupon Id", 400));
...
export const adminGetAllCoupans = TryCatch(
  async (req: Request, res: Response, next: NextFunction) => {
...
export const adminDeleteCoupan = TryCatch(
  async (req: Request, res: Response, next: NextFunction) => {
    const { coupon } = req.params;
    const oneCoupan = await Coupon.findOne({ coupon });

    if (!oneCoupan) return next(new ErrorHandler("Invalid Coupon Id", 400));
